...
exports.WriterClient = void 0;
const gax = require("google-gax");
const v1_1 = require("../v1");
const stream_types_1 = require("./stream_types");
const stream_connection_1 = require("./stream_connection");
/**
...
class WriterClient {
    constructor(opts) {
        ...
        this._client = new v1_1.BigQueryWriteClient({
            ...baseOptions,
            ...opts,
        });
        this._connections = {
     ...
     */
    async createWriteStream(request, options) {
     ...
     */
    async createWriteStreamFullResponse(request, options) {
        await this.initialize();
        const { streamType, destinationTable } = request;
        const createReq = {
            parent: destinationTable,
            writeStream: {
                type: (0, stream_types_1.streamTypeToEnum)(streamType),
            },
        };
     ...
     * @returns {Promise<WriteStream>}} - The promise which resolves to the WriteStream.
     */
    async getWriteStream(request, options) {
        await this.initialize();
     ...
     */
    async createStreamConnection(request, options) {
        await this.initialize();
        const { streamId, streamType, destinationTable } = request;
        try {
            const fullStreamId = await this.resolveStreamId(streamId, streamType, destinationTable);
            const streamConnection = new stream_connection_1.StreamConnection(fullStreamId, this, options);
    ...
    async resolveStreamId(streamId, streamType, destinationTable) {
        if (streamId && streamId !== '') {
            if (streamId === stream_types_1.DefaultStream) {
                if (destinationTable !== '') {
        ...
        throw new Error('streamId or destinationTable required to create write stream');
    }
    close() {
     ...
     */
    async batchCommitWriteStream(request) {
        await this.initialize();
     ...
     */
    async finalizeWriteStream(request) {
        await this.initialize();
...
//# sourceMappingURL=writer_client.js.map